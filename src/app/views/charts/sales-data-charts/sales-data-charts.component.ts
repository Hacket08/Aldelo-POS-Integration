...
@Component({
  selector: 'app-sales-data-charts',
  templateUrl: './sales-data-charts.component.html',
  styleUrls: ['./sales-data-charts.component.scss']
})
export class SalesDataChartsComponent implements OnInit {

  documentType: string = 'OrderHeaders';
  salesdata: SalesData[] = [];
  getdata: SalesData[] = [];
  // getInsHours: number[] = [];
  uniqueInsHours: number[] = [];
  ...
  private fetchSalesData(): void {
    this.apiservice.getData(this.documentType, 'GetAccumulatedSalesData', `${this.currentdate}/OLD TOWN`).subscribe(
      (data: any) => {
        this.salesdata = data;
        this.uniqueInsHours = Array.from(new Set(this.salesdata.map(salesData => salesData.ins_Hour)));
  ...
  private setupMainChart(): void {
    ...
    // this.uniqueInsHours = [ ...this.getInsHours];
  ...
  private createChartDatasets(): any[] {
    ...
    this.uniqueInsHours.forEach(hour => {
      uniqueInsBranchCode.forEach(branchCode => {
