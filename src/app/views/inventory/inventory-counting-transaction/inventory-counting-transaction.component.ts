...
export class InventoryCountingTransactionComponent implements OnInit {
  ...
  async addTransaction() {
    ...
    let docnum = await this.apiservice.getDataAsync('InventoryCount', 'GetMaxId');
  ...
  async editTransaction(documentnumber: string) {
    ...
    let response = await this.apiservice.getDataAsync('InventoryCount', 'GetTransaction', documentnumber);
  ...
  async submitTranaction() {
    ...
    if (this.docstatus !== -1) {
      let response = await this.apiservice.getDataAsync('InventoryCount', 'GetTransaction', this.documentid);
  ...
  performTransaction(action: string, title: string, text: string, icon: any, status: number) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        this.apiservice.postData(trans, 'InventoryCount', 'UpdateTransactionStatus')
          .subscribe((response: any) => {
            this.documentid = response.ins_InventoryCountID;
            this.docnum = response.ins_DocNum;
